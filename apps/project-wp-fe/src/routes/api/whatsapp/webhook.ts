...
async function processChange(wabaId: string, field: string, value: unknown) {
	switch (field) {
		...
		case "account_review_update":
			await handleAccountReviewUpdate(
				wabaId,
				value as AccountReviewUpdateValue,
			);
			break;

		case "phone_number_name_update":
			await handlePhoneNumberNameUpdate(
				wabaId,
				value as PhoneNumberNameUpdateValue,
			);
			break;
		...
		case "phone_number_quality_update":
			await handlePhoneNumberQualityUpdate(
				wabaId,
				value as PhoneNumberQualityUpdateValue,
			);
			break;
